...
This pipeline was designed by JASON CHARLES DAY as part of his MSc Thesis at The Centre for Bioinformatics and Computational Biology, Stellenbosch Univeristy, South Africa.

This pipeline is designed to discover high quality SNPs with the potential for use as KASP markers. The concept is split into two seperate pipelines, both producing a set of sequences containing the relevant SNPs with surrounding sequence in a format that is suitable for KASP primer design submission as requried by Barenbrug NZ. The process encapsulates and automates all steps of the variant calling and KASP marker identification process including

1. Basic pre-processing of sequencing reads 
2. Alignment of processed reads to a reference genome (Minimap2 for long reads and BWA MEM for short reads)
3. Sorting and Indexing of resulting BAM files using Samtools
4. Variant calling using Freebayes
5. Variant filtering using BCFtools
6. Extraction and generation of KASP markers + surrounding sequence using pysam
7. Standard end-run bioinformatics reporting using MultiQC
...
Input: raw short- or long-read FASTQ
Output: main/results/

Stream 2: EVAK_bam_pipe.nf
Input: pre-aligned BAM files
...
Short-read example:
id,single_end,read1,read2
A2_test_NEA56_Contig1,false,/home/jasonday/Barenbrug_Nextflow_Pipeline/main/data/A2_test_R1.fastq.gz,/home/jasonday/Barenbrug_Nextflow_Pipeline/main/data/A2_test_R2.fastq.gz

Long-read example:
id,single_end,read1,read2
NEA6_long,true,/home/jasonday/Barenbrug_Nextflow_Pipeline/main/data/NEA6_long.fastq.gz,
...
Example:
id,bam_path
NEA56_contig_7_bam_freebayes102,/home/jasonday/Barenbrug_Nextflow_Pipeline/main/data/ptg000007l_sorted.bam

3. OUTPUTS

STREAM 1 DEFAULT OUTPUT DIRECTORY:
main/results/
...
STREAM 2 DEFAULT OUTPUT DIRECTORY:
main/results_from_bam/
...
- --depth_multiplier         Upper depth threshold = multiplier × mean depth
- --flanking_region          Number of flanking bases around SNP (default 100)

6. EXECUTION DEPENDANCIES
